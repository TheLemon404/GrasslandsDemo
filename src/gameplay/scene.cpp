...
void Scene::PlaceTreesPseudoRandom(std::vector<Transform>& transforms, glm::vec2 terrainDimensions, glm::ivec2 gridDimensions, float placementPercent, float scale, float heightOffset) {
    for(int i = 0; i < gridDimensions.x; i++) {
        for(int j = 0; j < gridDimensions.y; j++) {
            ...
            if(percent <= placementPercent) {
                ...
                float distance = glm::distance(pos, glm::vec3(0.0f));
                if(distance <= 150.0f || distance > 250) continue;
